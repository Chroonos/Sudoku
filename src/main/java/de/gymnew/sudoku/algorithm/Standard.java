...
public class Standard implements Algorithm {
	...
	private void tryRandom() throws InterruptedException {
		...
		for (int k = 2; k < 10; k++) {
			for (int i = 0; i < 9; i++) {
				for (int j = 0; j < 9; j++) {
					if (sudoku.getField(i, j).countNotes() == k) {
						Byte[] notes = sudoku.getField(i, j).getNotes().toArray(new Byte[k]);
						int l = 0;
						while (l < k) {
							byte b = notes[l];
							sudoku = clone.clone();
	...
	private boolean isSolvable() {
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				if (sudoku.getField(i, j).countNotes() == 0 && sudoku.getField(i, j).getValue() == 0)
					return false;
			}
		}
		return true;
