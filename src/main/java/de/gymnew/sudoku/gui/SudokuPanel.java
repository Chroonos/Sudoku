	...
	protected void paintComponent(Graphics g) {
		...
		for(int block_x = 0; block_x < 3; block_x++){
			for(int block_y = 0; block_y < 3; block_y++){
				...
				g.fillRect(base_x - (BLOCK_SEPARATOR_WIDTH*scale), base_y - (BLOCK_SEPARATOR_WIDTH*scale), (BLOCK_SEPARATOR_WIDTH*2+FIELD_SIZE*3)*scale, (BLOCK_SEPARATOR_WIDTH*2+FIELD_SIZE*3)*scale);
				...
				for(int field_x = 0; field_x < 3; field_x++){
					for(int field_y = 0; field_y < 3; field_y++){
						...
						g.drawChars(new char[]{(""+sudoku.getField(field_x+3*block_x, field_y+3*block_y).getValue()).charAt(0)},
								0, 1, x+scale, y+field_size-scale);
					}
				}
			}
		}
		...
		g.drawChars(new char[]{'9'}, 0, 1, (OFFSET_SIDE+BLOCK_SEPARATOR_WIDTH)*scale+10, (OFFSET_TOP+BLOCK_SEPARATOR_WIDTH+FIELD_SIZE)*scale-10);
	}
